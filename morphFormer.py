...
import copy
import numpy as np
from HyperchannelViT import LCFE_Fusion
...
class MorphFormer(nn.Module):
    def __init__(self, FM, NC, num_classes, patch_size, near_band=1, mode='None'):
    ...
    def forward(self, x1):
        if self.mode == 'LCFE':
            x1 = self.LCFE_X(torch.as_tensor(x1).permute(0, 1, 4, 2, 3))  # (batch, patch_num, patch_size, patch_size, near_band)
            x1 = x1.permute(0, 2, 1, 3, 4)
        else:
            # do not use LCFE to enhance the model
